...
def get_nn_superpixel_ccrs(query_idx, segment_divisions=[30], use_box=False):
	"""
	Carve out the superpixel precisely
	"""
...
def save(occ_type, accs, iterations, epochs):
	...
	df['Epoch']  = epochs
	df['NumSpsUsed'] = NUM_SPS_USE
	df['PrecIncluded'] = sum(avg_size) / len(avg_size) 
	df.to_csv('data/NewTrainingResults/Training_InclusionSPs_' + occ_type + '_' + str(NUM_SPS_USE) + '.csv')
...
#     Get sp data
#     sp_results = get_greedy_superpixel_ccrs()
	...
	# Readjust CAM
	cam_torch_img = resize_cam_with_sp(pixel_cam, total_sp_blacked_out, train_on_segment)
	
	# Get FAM
	_, pixel_fam = get_num_cam_pixels_to_occlude(cam_type='FAM')
	fam_torch_img = resize_cam_with_sp(pixel_fam, total_sp_blacked_out, train_on_segment)
	...
	return sp_torch_img, cam_torch_img, fam_torch_img, rand_torch_img, total_sp_blacked_out
...
for NUM_SPS_USE in [0, 2, 4, 9, 14, 20, 25]:

	print('=====================================================')
	print("Training on This Number of Superpixels:", NUM_SPS_USE)
	print('=====================================================')
	...
	netC_retrain_cam, _ = collect_model(DEVICE)
	netC_retrain_fam, _ = collect_model(DEVICE)
	...
	optimizer_cam = torch.optim.SGD(netC_retrain_cam.parameters(), lr=LEARNING_RATE,
									momentum=MOMENTUM, weight_decay=WEIGHT_DECAY)
	optimizer_fam = torch.optim.SGD(netC_retrain_fam.parameters(), lr=LEARNING_RATE,
									momentum=MOMENTUM, weight_decay=WEIGHT_DECAY)
		...
		for i, data in enumerate(train_loader):
			...
			with torch.no_grad():
				...
				for k in range(len(imgs)):
					...
					sp_torch_img, cam_torch_img, fam_torch_img, rand_torch_img, a_size = return_occluded_imgs(
						train_on_segment=TRAIN_ON_SEGMENT, perc_use=PERC_USE)

					sp_torch_imgs[k] = sp_torch_img[0]
					cam_torch_imgs[k] = cam_torch_img[0]
					fam_torch_imgs[k] = fam_torch_img[0]
			...
			logits, _, _ = netC_retrain_cam(cam_torch_imgs.to(DEVICE))
			classify_loss = cce_loss(logits, labels)
			classify_loss.backward()
			optimizer_cam.step()

			logits, _, _ = netC_retrain_fam(fam_torch_imgs.to(DEVICE))
			classify_loss = cce_loss(logits, labels)
			classify_loss.backward()
			optimizer_fam.step()
			...
			if current_iter == 10 or current_iter == 50 or current_iter % 100 == 0:
				...
				acc_cam = evaluate_test(test_loader, netC_retrain_cam.eval(), test_dataset, DEVICE)
				acc_fam = evaluate_test(test_loader, netC_retrain_fam.eval(), test_dataset, DEVICE)
				acc_rand = evaluate_test(test_loader, netC_retrain_rand.eval(), test_dataset, DEVICE)
				print("Meta Data:", current_iter, acc_sp, acc_cam, acc_fam, acc_rand)
				...
				print([acc_sp, acc_cam, acc_fam, acc_rand])
